...
E_DEVLET_GRUP_ADI = "öğrenciler"
OLUSTURULAN_EXCEL_DOSYASI_ADI = "ogrenciler.xlsx"
TIMEOUT = 15
TARAYICIYI_GIZLE = False
...
class MWindow(QMainWindow):
    ...
    def kaydet(self):
        ...
        if self.ekleGuncellePencere.widgetKaydet.text() == "Kaydet":
            ...
            if len(numaraKayitli) > 0 or len(tcNoKayitli) > 0 or len(hesKayitli) > 0:
                self.ekleGuncellePencere.hata.show()
            ...
            try:
                self.yukleniyorAnimasyonuBaslat()
                th = Thread(target=self.eDevlettenGrupGuncelleSayfasiAc, args=(E_DEVLET_GRUP_ADI, True, ogrenci))
                th.start()
            except:
        ...
        #Güncelleme sayfasındayız.              
            ...
            #Hes değişti. E-devletten hes kodu eklenecek. Eklenirse veritabanına kaydedilecek.
                ...
                #Yeni resim eklenmediyse eski öğrenci güncellenecek.
                    ...
                    self.yukleniyorAnimasyonuBaslat()
                    th = Thread(target= self.eDevlettenGrupGuncelleSayfasiAc, args=(E_DEVLET_GRUP_ADI, True, ogrenci, True))
                    th.start()
    ...
    def eDevletGirisiYap(self, tcKimlik="", sifre="", gizliPencere = TARAYICIYI_GIZLE):     
        try:
    ...
    def grupOlustur(self, grupAdi, tamDosyaYolu, ogrenciler, aciklama="" , varolanGrubaEkle= False):
        ...
        try:
            ...
            if not varolanGrubaEkle:
                ...
                self.eDevlettenGrupGuncelleSayfasiAc(E_DEVLET_GRUP_ADI, browser=True)
    ...
    def topluKaydet(self):
        ...
        if os.path.exists(self.dosyaYoluLineEdit.text()) and os.path.exists(self.fotoYoluLineEdit.text()):
            ...
            if len(vt.DB.sinif_isimleri()) == 0:
                th = Thread(target=self.grupOlustur, args=(E_DEVLET_GRUP_ADI, yol, ogrenciler))
                th.start()
            else:
                th = Thread(target=self.grupOlustur, args=(E_DEVLET_GRUP_ADI, yol, ogrenciler, "", True))
                th.start()
