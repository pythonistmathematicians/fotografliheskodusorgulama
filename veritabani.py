...
class DB:
    ...
    @staticmethod
    @resimdonustur
    def tc_sonu_ile(son_uc, ad_ilk_iki, soyad_ilk_iki):
    
        with DB.veritabani() as db:
            son_uc = "%{}".format(son_uc)
            ad_ilk_iki = "{}%".format(ad_ilk_iki)
            soyad_ilk_iki = "{}%".format(soyad_ilk_iki)

            q = """SELECT * FROM ogrenciler WHERE tc_no LIKE ? AND isim LIKE ? AND soyad LIKE ?"""
            cur = db.cursor()
            cur.execute(q, (son_uc, ad_ilk_iki, soyad_ilk_iki))
            return cur.fetchall()
